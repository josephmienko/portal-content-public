...
sp_call <- stored_procedure('ooh_flow_entries_counts')
sp_query <- sqlQuery(annie, sp_call)
clean_query <- cr_clean(sp_query, select = 'qry_type_poc1', date.type = 2) %>%
    mutate(nice.entries = comma(number.of.entries))
# code for highlights

current_data <- filter(clean_query, cohort.period == max(cohort.period))
min_data <- filter(clean_query, number.of.entries == min(number.of.entries)) %>%
                filter(cohort.period == max(cohort.period))
max_dat <- filter(clean_query, number.of.entries == max(number.of.entries)) %>%
                filter(cohort.period == max(cohort.period))
...
<div class="stat">
    ...
    <p>`r current_data$nice.entries`</p>
</div>

<div class="stat">
    <h4>Low: `r year(min_data$cohort.period)`</h4>
    <p>`r min_data$nice.entries`</p>
</div>

<div class="stat">
    <h4>High: `r year(max_dat$cohort.period)`</h4>
    <p>`r max_dat$nice.entries`</p>
</div>
