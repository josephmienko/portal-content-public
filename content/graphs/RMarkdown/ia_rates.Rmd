...
sp_call <- stored_procedure('ia_trends_rates')
sp_query <- sqlQuery(con, sp_call)
clean_query <- cr_clean(sp_query, date.type = 2)

# code for highlighta

recent_data <- filter(clean_query, date == max(date))

min_data <- filter(clean_query, opened.investigations.and.assessments == min(opened.investigations.and.assessments))
max_data <- filter(clean_query, opened.investigations.and.assessments == max(opened.investigations.and.assessments))
...
<div class="stat">
    <h4>Recent: `r year(recent_data$date)`</h4>
    <p>`r round(recent_data$opened.investigations.and.assessments)`</p>
</div>

<div class="stat">
    <h4>Low: `r year(min_data$date)`</h4>
    <p>`r round(min_data$opened.investigations.and.assessments)`</p>
</div>

<div class="stat">
    <h4>High: `r year(max_data$date)`</h4>
    <p>`r round(max_data$opened.investigations.and.assessments)`</p>
</div>
